...
const option = {
  title: { text: 'BTC/USD', left: 'center' },
  tooltip: { trigger: 'axis' },
  xAxis: { type: 'category', data: [], scale: true, boundaryGap: false },
  yAxis: { scale: true },
  series: [{ id: 'candlestick', type: 'candlestick', name: 'BTC', data: [] }],
  dataZoom: [
...
document.querySelectorAll("ul li button").forEach(btn => {
  btn.addEventListener("click", () => {
    const ruta = "criptomonedas/" + btn.dataset.path;
...
// -------------------- CARGA INICIAL --------------------
cargarCSV("criptomonedas/archivos/btc.csv", "BTC");
